...
let FileSystem = null;
try {
    // expo-file-system is not available on web; guard import
...
class DataManagerClass {
    ...
    async _initSessionCache() {
        try {
            ...
            // Create fresh cache directory for this session (native only)
    ...
    async _downloadAndCacheImage(url) {
        ...
        // On web, skip file caching and return original URL
        if (Platform.OS === 'web' || !FileSystem) {
            return url;
        }
    ...
    if (processedData.userInfo) {
            ...
            if (Array.isArray(processedData.userInfo.items)) {
                for (const item of processedData.userInfo.items) {
        ...
        if (Array.isArray(processedData.shop)) {
            for (const item of processedData.shop) {
                if (item.imageUrl) {
                    item.imageUrl = await this._downloadAndCacheImage(item.imageUrl);
                }
                if (item.previewUrl) {
        ...
        if (Array.isArray(processedData.badges)) {
            for (const badge of processedData.badges) {
                if (badge.imageUrl) {
        ...
        if (Array.isArray(processedData.challenges)) {
            for (const challenge of processedData.challenges) {
        ...
        if (Array.isArray(processedData.disciplines)) {
            for (const area of processedData.disciplines) {
    ...
    async loadAppData() {
        ...
        try {
            ...
            try {
                // Load all ranking types sequentially to comply with refresh-token constraints
